...
import { Invoice } from 'src/app/oprup-invoices/invoice';
import { InvoiceService } from 'src/app/oprup-invoices/invoice.service';
...
export class CreateSalesInvoiceComponent implements OnInit {
  ...
  public count() {
    this.salesInvoiceService.countSalesInvoice().subscribe((data) => {
      this.salesInoviceData.salesInvoiceNumber = `inv-${data + 1}`;
    });
  }
  ...
  public onAddSaleInvoice(): void {
    this.salesInvoiceService.addInvoice(this.salesInoviceData).subscribe(
      (response: SalesInvoice) => {
        ...
        this.invoiceItems.forEach((element) => {
          this.salesInvoiceService
            ...
            .subscribe((response: InvoiceItems) => {});
        }),
          (error: HttpErrorResponse) => {
        ...
        this.journalService
          .addJournalVoucher({
            ...
            dateVoucher: this.salesInoviceData.date,
            costCenter: {
              costCenterId: this.salesInoviceData.costCenter.costCenterId,
            },
          })
          .subscribe((response) => {
            ...
            if (this.salesInoviceData.salesInvoiceType === 'debit')
              journalDetailsfn(
                response.journalVoucherId,
                this.salesInoviceData.total,
                0,
                this.salesInoviceData.clientAccount.accountId,
                null
              );
            if (this.salesInoviceData.salesInvoiceType === 'cash')
              journalDetailsfn(
                response.journalVoucherId,
                this.salesInoviceData.total,
                0,
                this.salesInoviceData.cashAccount.accountId,
                null
              );
            journalDetailsfn(
              response.journalVoucherId,
              0,
              this.salesInoviceData.tax,
              this.salesInoviceData.taxAccount.accountId,
              this.salesInoviceData.costCenter.costCenterId

            );
            journalDetailsfn(
              response.journalVoucherId,
              0,
              this.salesInoviceData.amount,
              this.salesInoviceData.salesAccount.accountId,
              this.salesInoviceData.costCenter.costCenterId
            );
          });
        ...
        this.invoiceService.addInvoice({
          customerName: 'امدادات العطاء',
          taxNumber: this.salesInoviceData.taxNumber,
          invoiceDate: this.salesInoviceData.date,
          tax: this.salesInoviceData.tax,
          price: this.salesInoviceData.total,
          deleteFlag: 1,
          salesInvoice:{
  ...
  calculateInvoice(amount=0, total = 0) {
    this.rows.value.forEach((element) => {
      ...
      if (amount==null || amount==0 ){
       ...
       this.salesInoviceData.tax = (
        this.salesInoviceData.amount * 0.15
      ).toString();
        this.salesInoviceData.total = total+Number(this.salesInoviceData.tax );
      }
      else
      {
        this.salesInoviceData.amount = amount;
...
//     this.salesInoviceData.amount = amount;
//    // total += element.salesInvoiceItemTotal;
//     this.salesInoviceData.total = amount;
//   });
//   this.salesInoviceData.tax=(0).toString();
//   // this.salesInoviceData.tax = (
//   //   this.salesInoviceData.amount * 0.15
//   // ).toString();
// }
