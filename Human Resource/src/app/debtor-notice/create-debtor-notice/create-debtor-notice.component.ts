...
@Component({
  selector: 'app-create-debtor-notice',
  templateUrl: './create-debtor-notice.component.html',
  styleUrls: ['./create-debtor-notice.component.scss']
})
export class CreateDebtorNoticeComponent implements OnInit {
  ...
  displayColumns = [
    ...
    'numberOfEmployees',
    'operationFee',
    'debtorNoticeItemValue',
    'debtorNoticeItemTotal',
    'monthlyOperationFee',
    'taxRate',
    'delete',
  ];
  ...
  form: FormGroup = this.fb.group({
    ...
    debtorNoticeNumber: new FormControl(''),
    debtorNoticeType: new FormControl(''),
  ...
  constructor(
    ...
    private debtorNoticeService: DebtorNoticeService,
  ...
  count(){
    this.debtorNoticeService.count().subscribe(data=>{
      this.debtorNoticeData.debtorNoticeNumber=(data+1).toString()
    })
  }
  ...
  //     this.debtorNoticeData.salesInvoiceNumber = `inv-${data + 1}`;
  //   });
  // }
  ...
  public onAddSaleInvoice(): void {
    this.debtorNoticeService.addDebtorNotice(this.debtorNoticeData).subscribe(
      (response) => {
        console.log('response', response);
        this.rows.value.forEach((element) => {
          element.debtorNoticeId = response.debtorNoticeId;
          this.invoiceItems.push({
            debtorNoticeItemValue: element.debtorNoticeItemValue,
            debtorNoticeItemTotal: element.debtorNoticeItemTotal,
            ...
            debtorNotice: {
              debtorNoticeId: element.debtorNoticeId,
            },
            item: {
        ...
        this.invoiceItems.forEach((element) => {
          this.debtorNoticeService
            .addInvoiceItems(element)
            .subscribe((response) => {});
        }),
          (error: HttpErrorResponse) => {
            console.log(this.debtorNoticeData);
        ...
        this.journalService
          .addJournalVoucher({
            ...
            dateVoucher: this.debtorNoticeData.date,
            costCenter: {
              costCenterId: this.debtorNoticeData.costCenter.costCenterId,
            },
          })
          .subscribe((response) => {
            ...
            //   debit: this.debtorNoticeData.total.toString(),
            //   credit: '0',
            //   journalVoucherDetailsId: '',
            //   description: '',
            //   chartAccounts: {
            //     accountId: this.debtorNoticeData.clientAccount.accountId
            //   }
            // }
            ...
            const journalDetailsfn=(journalVoucherId,debit,credit,accountId,costCenter)=>{
              ...
              journalDetailsfn(
                ...
                this.debtorNoticeData.total,
                0,
                this.debtorNoticeData.clientAccount.accountId,
                null
              );
            ...
            journalDetailsfn(
              response.journalVoucherId,
              0,
              this.debtorNoticeData.tax,
              this.debtorNoticeData.taxAccount.accountId,
              this.debtorNoticeData.costCenter.costCenterId

            );
            journalDetailsfn(
              response.journalVoucherId,
              0,
              this.debtorNoticeData.amount,
              this.debtorNoticeData.salesAccount.accountId,
              this.debtorNoticeData.costCenter.costCenterId
            );
          });
...
items.monthytotal = gelAll(empNum,oprFee)
  ...
  calculateInvoice(amount=0, total = 0) {
    this.rows.value.forEach((element) => {
      amount += element.debtorNoticeItemValue;
      if (amount==null || amount==0 ){
        amount += element.monthlyOperationFee;
        this.debtorNoticeData.amount = amount;
       total += element.monthlyOperationFee;
       this.debtorNoticeData.tax = (
        this.debtorNoticeData.amount * 0.15
      ).toString();
        this.debtorNoticeData.total = total+Number(this.debtorNoticeData.tax );
      }
      else
      {
        this.debtorNoticeData.amount = amount;
        total += element.debtorNoticeItemTotal;
        this.debtorNoticeData.total = total;
        this.debtorNoticeData.tax = (
          this.debtorNoticeData.amount * 0.15
        ).toString();
      }
...
//     this.debtorNoticeData.amount = amount;
//    // total += element.debtorNoticeItemTotal;
//     this.debtorNoticeData.total = amount;
//   });
//   this.debtorNoticeData.tax=(0).toString();
//   // this.debtorNoticeData.tax = (
//   //   this.debtorNoticeData.amount * 0.15
//   // ).toString();
// }
